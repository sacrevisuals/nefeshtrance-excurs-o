...
generateBtn.addEventListener("click", () => {
  ...
  // Adicionar texto com sistema híbrido (texto livre + auto-ajuste)
  const text = infoText.value.trim() || MOCKUP_CONFIG.textArea.freeText;
  if (text) {
    ctx.save();
    ...
    // 🚌 SISTEMA HÍBRIDO: Quebra automática de texto livre
    const lines = text.split("\n").map((line) => line.trim()).filter(line => line !== "");

    // Preparar linhas sem lógica de lote automático
    const preparedLines = lines.map(line => ({
      text: line.toUpperCase(),
      isPhone: /\(?\d{2}\)?\s?\d{4,5}[-\s]?\d{4}/.test(line)
    }));
